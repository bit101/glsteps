...
const (
	...
	// shaders and vertices are now external
	vertexShaderSource   = "step07/vertex_shader.gl"
	fragmentShaderSource = "step07/fragment_shader.gl"
	verticesSource       = "step07/vertices.json"
)
...
// Main loads shaders and vertices from external files
func Main() {
	runtime.LockOSThread()
	initGlfw()
	window := createWindow("Step 07")
...
func initVertices() uint32 {
	// read the vertices file and unmarshal into the vertices array.
...
// compile now loads external shader source
func compileShader(path string, shaderType uint32) (uint32, error) {

	// read source file and get string containing source
